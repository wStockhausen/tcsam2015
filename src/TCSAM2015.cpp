...
void model_parameters::setDevs(dvar_matrix& devs, param_init_bounded_vector_vector& pDevs, int debug, ostream& cout)
{
...
void model_parameters::setDevs(int debug, ostream& cout)
{
    if (debug>=dbgAll) cout<<"starting setDevs()"<<endl;
    setDevs(devsLnR, pDevsLnR,debug,cout);
    setDevs(devsS1, pDevsS1,debug,cout);
    setDevs(devsS2, pDevsS2,debug,cout);
    setDevs(devsS3, pDevsS3,debug,cout);
    setDevs(devsS4, pDevsS4,debug,cout);
    setDevs(devsS5, pDevsS5,debug,cout);
    setDevs(devsS6, pDevsS6,debug,cout);
    
    setDevs(devsLnC, pDevsLnC,debug,cout);
    if (debug>=dbgAll) cout<<"finished setDevs()"<<endl;
...
void model_parameters::initPopDyMod(int debug, ostream& cout)
{
    ...
    setDevs(debug,cout);//set devs vectors
...
void model_parameters::calcPriors(NumberVectorInfo* ptrVI,param_init_number_vector& pv, int debug, ostream& cout)
{
    if (ptrVI->getSize()){
        dvar_vector tmp = pv;
...
void model_parameters::calcPriors(BoundedNumberVectorInfo* ptrVI,param_init_bounded_number_vector& pv, int debug, ostream& cout)
{
    if (ptrVI->getSize()){
        dvar_vector tmp = pv;
        dvar_vector pri = ptrVI->calcLogPriors(tmp);//ln-scale prior (NOT NLL!)
...
void model_parameters::calcPriors(BoundedVectorVectorInfo* ptrVVI, param_init_bounded_vector_vector& pm, int debug, ostream& cout)
{
    if (ptrVVI->getSize()){
...
void model_parameters::calcPriors(DevsVectorVectorInfo* ptrVVI, dvar_matrix& pm, int debug, ostream& cout)
{
    if (ptrVVI->getSize()){
        dvector wts = ptrVVI->getPriorWgts();
        for (int i=1;i<=pm.indexmax();i++) {
